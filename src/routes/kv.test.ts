...
describe("kvGetHandler", () => {
  it("should return valid response", async () => {
    ...
    const req = buildRequest(new Request("https://example.com/kv/test"), env, ctx) as KvGetRequest;
    req.parsedBody = {
      key: "test"
    }
...
describe("kvPutHandler", () => {
  it("should set and return valid response", async () => {
    const ctx = new ExecutionContext();
    const env = getMiniflareBindings() as Env;
    const req = buildRequest(new Request("https://example.com/kv/test", {
      method: "PUT",
      body: JSON.stringify({ key: "test", value: "test value" })
    }), env, ctx) as KvPutRequest;
    req.parsedBody = {
      key: "test",
      value: "test value"
    }
  ...
  it("should throw validation error", async () => {
    ...
    expect(res?.status).toBe(400);
    expect(await res?.json()).toEqual({ status: 400, error: "key: Required, value: Required" });
  });
});
